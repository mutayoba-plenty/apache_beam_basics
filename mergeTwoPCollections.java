...
public class mergeTwoPCollections {
	public static void main(String[] argvs) {
		...
		PCollection<byte[]> getDataAsByteStream2 = p.apply(KinesisIO.read()
			     .withStreamName(streamName)
			     .withInitialPositionInStream(InitialPositionInStream.LATEST)
			     .withAWSClientsProvider(aws_access_key_id, aws_secret_access_key, Regions.US_EAST_1))
		.apply(
		...
		PCollectionList<byte[]> collectionList = PCollectionList.of(getDataAsByte).and(getDataAsByteStream2);
		PCollection<byte[]> MergeList = collectionList.apply(Flatten.<byte[]>pCollections());
